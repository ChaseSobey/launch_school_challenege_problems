...
def sum_even_number_row(row_number)
  ...
  (1..row_number).each do |current_row_number|
    ...
    start_integer = rows.last.last + 2 #returns the last integer of the last row in the rows array, then adds 2 to it and assigns that value to start_integer
  end
  
  rows.last.each { |value| final_row_sum += value }
